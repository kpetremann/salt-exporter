...
func insertItemIntoSlice(items []Item, item Item, index int) []Item {
	...
	// Allocate a fresh slice (len == cap) so that any filterItems goroutine
	// that captured the old slice header never shares a backing array with
	// this new slice, preventing nil-pointer races in FilterValue().
	result := make([]Item, len(items)+1)
	copy(result, items[:index])
	result[index] = item
	copy(result[index+1:], items[index:])
	return result
}
...
func removeItemFromSlice(i []Item, index int) []Item {
	...
	// that captured the old slice header never shares a backing array with
	// this new slice, preventing nil-pointer races in FilterValue().
	result := make([]Item, len(i)-1)
	copy(result, i[:index])
	copy(result[index:], i[index+1:])
	return result
}
...
func removeFilterMatchFromSlice(i []filteredItem, index int) []filteredItem {
	...
	// Allocate a fresh slice (len == cap) for consistency with removeItemFromSlice.
	result := make([]filteredItem, len(i)-1)
	copy(result, i[:index])
	copy(result[index:], i[index+1:])
	return result
}
